...
if ( ! function_exists( 'consultx_setup' ) ) :
	...
	function consultx_setup() {
		 ...
		 */
		load_theme_textdomain( 'consultx', get_template_directory() . '/languages' );
		...
		add_theme_support( 'custom-background', apply_filters( 'consultx_custom_background_args', array(
...
function consultx_scripts() {
	...
	wp_enqueue_script( 'isotope', get_theme_file_uri( '/assets/js/isotope.js' ), 
		array( ) 							      					   						   );
	//owl-carousel js
	wp_enqueue_script( 'owl-carousel', get_theme_file_uri( '/assets/js/owl.carousel.js' ), 
		array( ) 							      					   						   );
	//slick js
...
function consultx_widgets_init() {
	...
	register_sidebar( array(
		...
		'after_widget'  => '</section>',
		'before_title'  => '<h2 class="logo-footer"><span>',
		'after_title'   => '</span></h2>',
	) );
